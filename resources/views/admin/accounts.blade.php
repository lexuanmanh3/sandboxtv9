...
@section('title', 'tv9tech - Quản lý tài khoản hệ thống')
...
@section('content')
      ...
      <div class="account-table-wrap">
        <table class="account-table">
          ...
          <tbody data-account-table-body>
            @forelse ($users as $user)
              ...
              <tr data-account-row>
                <td class="td-checkbox">
                  <input type="checkbox" class="account-table-checkbox bulk-item-checkbox" value="{{ $user->id }}" @disabled($isRootAdmin || $isCurrentUser)>
                </td>
                <td>
            ...
            @empty
              <tr>
                <td class="account-empty" colspan="10">Chưa có tài khoản nào trong hệ thống.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
  ...
  {{-- THANH TÁC VỤ NỔI KHI CHỌN NHIỀU TÀI KHOẢN --}}
  ...
  <div class="account-modal" data-account-modal="bulk-delete" hidden>
    <div class="account-modal__backdrop" data-account-modal-close></div>
    <section class="account-modal__dialog account-modal__dialog--sm" role="dialog" aria-modal="true" aria-labelledby="bulkAccountDeleteTitle">
      <header class="account-modal__header">
        <h2 id="bulkAccountDeleteTitle">Xóa nhiều tài khoản</h2>
        <button type="button" class="account-modal__close" data-account-modal-close aria-label="Đóng">×</button>
      </header>
      <form class="account-modal__body" method="POST" action="{{ route('admin.accounts.bulk-delete') }}" id="bulkAccountDeleteForm">
        @csrf
        <div id="bulkAccountDeleteHiddenInputs"></div>
        <p class="account-confirm-text" style="padding: 20px; margin: 0; color: var(--admin-slate-700);">
          Bạn có chắc muốn xóa <strong id="bulkAccountDeleteConfirmCount" style="color: #ef4444;">0</strong> tài khoản đã chọn? Thao tác này sẽ chuyển trạng thái tài khoản sang đã xóa.
        </p>
        <footer class="account-modal__footer">
          <button class="account-btn account-btn--muted" type="button" data-account-modal-close>Hủy</button>
          <button class="account-btn account-btn--danger" type="submit">Xóa các tài khoản</button>
        </footer>
      </form>
    </section>
  </div>
...
@push('scripts')
  <script src="{{ asset('backend/js/admin-accounts.js') }}?v={{ time() }}"></script>
@endpush
